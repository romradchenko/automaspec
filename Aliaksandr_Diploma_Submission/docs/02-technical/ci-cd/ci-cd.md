...
### Artifact Management and Debugging

To facilitate troubleshooting and provide a permanent record of code quality, the pipeline captures and stores the following artifacts after execution:

| Artifact Name | Job | Content | Retention |
|---------------|-----|---------|-----------|
| **security-audit-report** | Security Audit | JSON and text-based reports from `pnpm audit`. | 90 days |
| **coverage-report** | Quality Checks | Full Vitest HTML coverage reports for performance and quality tracking. | 90 days |
| **quality-checks-logs** | Quality Checks | Consolidated console output from typecheck, tests, lint (oxlint), and format (oxfmt) checks. | 90 days |
| **playwright-report** | E2E Tests | Full HTML report generated by Playwright including detailed test steps and browser logs. | 30 days |
| **e2e-test-results** | E2E Tests | Playwright traces, videos, and screenshots captured during failures, plus `test-results.json`. | 30 days |
| **vercel-build-artifacts** | Deploy | The compiled `.vercel/output` and detailed build logs (`vercel-build.log`). | 7-30 days |

All artifacts are uploaded using `actions/upload-artifact@v4`. Security, quality, and E2E results use `if: always()` to ensure evidence is available even when a job fails, drastically reducing time-to-resolve for CI issues.
...
| 4 | **Artifacts** | ✅ | Docker Images, Vercel Build Artifacts, Security Reports, Coverage, Playwright Traces/Reports, and Build Logs. |
